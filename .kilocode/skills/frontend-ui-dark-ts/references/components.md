...
const variants: Record<ButtonVariant, string> = {
	...
	secondary:
		'bg-neutral-bg3 text-text-primary border border-border hover:bg-neutral-bg4 hover:border-border-strong',
...
export const Card = forwardRef<HTMLDivElement, CardProps>(
	(
		...
		const baseClass = clsx(
			...
			paddings[padding],
			interactive &&
				'cursor-pointer hover:border-border-strong transition-colors duration-150',
			className,
		)
			...
			{open && (
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					<motion.div
						...
						className={clsx('relative w-full mx-4 glass-card p-6', sizes[size])}
					>
						{/* Close button */}
						<button
							onClick={onClose}
